...
set +x
export PIPELINE_IMAGE_URL
envsubst < deployment/deployment.yaml > deployment/templated-deployment.yaml
ibmcloud sat config version create --name "$BUILD_NUMBER" --config hello-world-multicloud-app --file-format yaml --read-config deployment/templated-deployment.yaml
